...
EmojiPlayer::EmojiPlayer(esp_lcd_panel_handle_t panel, esp_lcd_panel_io_handle_t panel_io)
{
    ...
    StartPlayer(MMAP_ASSETS_A_STAR_AAF, true, 15);
}
...
void EmojiWidget::SetEmotion(const char* emotion)
{
    ...
    static const std::unordered_map<std::string, Param> emotion_map = {
         {"bell",       {MMAP_ASSETS_A_BELL_AAF,      true, 25}},
         {"blinking",   {MMAP_ASSETS_A_BLINKING_AAF,  true, 10}},
         {"cookie",     {MMAP_ASSETS_A_COOKIE_AAF,    true, 24}},
         {"heart",      {MMAP_ASSETS_A_HEART_AAF,     true, 25}},
         {"sleep",      {MMAP_ASSETS_A_SLEEP_AAF,     true, 24}},
         {"snowman",    {MMAP_ASSETS_A_SNOWMAN_AAF,   true, 25}},
         {"star",       {MMAP_ASSETS_A_STAR_AAF,      true, 25}},
         {"elf",        {MMAP_ASSETS_A_WHOLE_ELF_AAF, true, 21}},
         {"wrong",      {MMAP_ASSETS_A_CROSS_AAF,     true, 24}},
         {"wrong2",      {MMAP_ASSETS_A_CROSS2_AAF,     true, 24}},
         {"happy",      {MMAP_ASSETS_A_HAPPY_AAF,     true, 24}},
         {"happy2",      {MMAP_ASSETS_A_HAPPY2_AAF,     true, 24}}
    };
    ...
    if (it != emotion_map.end()) {
        ...
        player_->StartPlayer(MMAP_ASSETS_A_BLINKING_AAF, true, 20);
    } else {
        ESP_LOGW(TAG, "Unknown emotion: %s", emotion);
        player_->StartPlayer(MMAP_ASSETS_A_CROSS_AAF, true, 20); // Default to blinking if unknown
    }
}
...
void EmojiWidget::SetStatus(const char* status)
{
    if (player_) {
        if (strcmp(status, "聆听中...") == 0) {
            player_->StartPlayer(MMAP_ASSETS_A_CROSS_AAF, true, 15);
        } else if (strcmp(status, "待命") == 0) {
            player_->StartPlayer(MMAP_ASSETS_A_HAPPY_AAF, true, 15);
        }
    }
}
